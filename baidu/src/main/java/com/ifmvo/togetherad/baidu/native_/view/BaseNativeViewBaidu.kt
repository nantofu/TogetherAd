...
abstract class BaseNativeViewBaidu : BaseNativeView() {
  ...
  override fun showNative(
    ...
    //Image
    getMainImageView()?.let {
      TogetherAd.mImageLoader?.loadImage(container.context, it, adObject.imageUrl)
    }
    getIconImageView()?.let {
      TogetherAd.mImageLoader?.loadImage(container.context, it, adObject.iconUrl)
    }
    getBaiduLogoImageView()?.let {
      TogetherAd.mImageLoader?.loadImage(container.context, it, adObject.baiduLogoUrl)
    }
    getAdLogoImageView()?.let {
      TogetherAd.mImageLoader?.loadImage(container.context, it, adObject.adLogoUrl)
    }
