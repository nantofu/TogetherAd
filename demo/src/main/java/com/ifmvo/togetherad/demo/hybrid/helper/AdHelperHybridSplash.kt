...
class AdHelperHybridSplash(
  ...
  private fun realLoadOnlyNative(
    ratioMap: LinkedHashMap<String, Int>,
    adProviderType: String
  ) {
    CsjProvider.Native.nativeAdType = AdSlot.TYPE_FEED
    adProvider?.getNativeAdList(activity = mActivity.get()!!, adProviderType = adProviderType,
      alias = mAlias, maxCount = 1, listener = object : NativeListener {
  ...
  private fun realLoadOnlySplash(
    ratioMap: LinkedHashMap<String, Int>,
    adProviderType: String
  ) {
    adProvider?.loadOnlySplashAd(activity = mActivity.get()!!, adProviderType = adProviderType,
      alias = mAlias, listener = object : SplashListener {
      override fun onAdFailed(
        providerType: String,
        failedMsg: String?
      ) {
  ...
  fun showAd(@NotNull container: ViewGroup): Boolean {
    ...
    if (mAdObject != null) {
      ...
      AdHelperNativePro.show(adObject = mAdObject, container = container,
