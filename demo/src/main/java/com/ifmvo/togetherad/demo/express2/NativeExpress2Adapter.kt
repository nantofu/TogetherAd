    ...
    return if (viewType == ITEM_VIEW_TYPE_CONTENT) {
      ContentViewHolder(
        LayoutInflater.from(parent.context)
          .inflate(R.layout.list_item_native_content, parent, false)
      )
    } else {
      AdViewHolder(
        LayoutInflater.from(parent.context).inflate(R.layout.list_item_native_ad, parent, false)
      )
    }
  }
  ...
  override fun onBindViewHolder(
    ...
    when (getItemViewType(position)) {
      ITEM_VIEW_TYPE_CONTENT -> {
        ...
        contentViewHolder.imageView.layoutParams.height =
          ScreenUtil.getDisplayMetricsWidth(contentViewHolder.imageView.context) * 9 / 16
        TogetherAd.mImageLoader?.loadImage(
          contentViewHolder.imageView.context,
          contentViewHolder.imageView, contentDataEntity.imgUrl
        )
      ...
      ITEM_VIEW_TYPE_AD -> {
        ...
        mActivity.get()?.let {
          AdHelperNativeExpress2.show(it, mList[position], adViewHolder.adContainer,
            NativeExpress2TemplateSimple(), object : NativeExpress2ViewListener {
