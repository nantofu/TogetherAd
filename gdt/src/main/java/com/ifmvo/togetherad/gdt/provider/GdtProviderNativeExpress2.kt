...
abstract class GdtProviderNativeExpress2 : GdtProviderNativeExpress() {
  ...
  override fun getNativeExpress2AdList(
    ...
    val mNativeExpressAD2 =
      NativeExpressAD2(activity, TogetherAdGdt.idMapGDT[alias], adLoaderListener)
    mNativeExpressAD2.setAdSize(
      GdtProvider.NativeExpress.adWidth,
      GdtProvider.NativeExpress.adHeight
    ) // 单位dp
     ...
     * 如果广告位仅支持图文广告，则无需调用
     */
    builder.setAutoPlayPolicy(
      GdtProvider.NativeExpress.autoPlayPolicyVideoOption2
    ) // WIFI 环境下可以自动播放视频
