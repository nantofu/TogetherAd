...
abstract class BaseNativeViewGdt : BaseNativeView() {
  ...
  override fun showNative(
    ...
    when (adObject.adPatternType) {
      AdPatternType.NATIVE_1IMAGE_2TEXT, AdPatternType.NATIVE_2IMAGE_2TEXT, AdPatternType.NATIVE_3IMAGE -> {
        ...
        getMainImageView()?.let {
          TogetherAd.mImageLoader?.loadImage(container.context, it, adObject.imgUrl)
        }
      }
    ...
    adObject.bindAdToView(
      container.context, getNativeAdContainer(), null, getClickableViews(),
      getCustomClickableViews()
    )
